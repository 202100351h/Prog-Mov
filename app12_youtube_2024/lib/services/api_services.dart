...
class APIService {
  Future<List<VideoModel>> getVideos() async {
    ...
    String path =
        "https://youtube.googleapis.com/youtube/v3/search?part=snippet&key=AIzaSyBuzuDZE9bPH0F70M-it5SQMenmTS9KD-w&maxResults=20&regionCode=PE";
  ...
  Future<VideoModel> fetchVideoDetails(String videoId, String channelId) async {
    final response = await http.get(
      Uri.parse(
          'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id=$videoId&key=AIzaSyBuzuDZE9bPH0F70M-it5SQMenmTS9KD-w'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      return VideoModel(
        ...
        viewCount:
            jsonData['items'][0]['statistics']['viewCount']?.toString() ?? '0',
  ...
  Future<String> fetchChannelAvatar(String channelId) async {
    final response = await http.get(
      Uri.parse(
          'https://www.googleapis.com/youtube/v3/channels?part=snippet&id=$channelId&key=AIzaSyBuzuDZE9bPH0F70M-it5SQMenmTS9KD-w'),
    );
